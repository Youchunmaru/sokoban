...
public class App {
    ...
    public static void main(String[] args ) throws SokobanException, IOException {
        ...
        if (checkLevel(name)) {
            ...
            //SokobanLevel sokoban = new SokobanLevel(path);
            Sokoban sokoban = new Sokoban(path);
            boolean playing = true;
            //Loop to get a input every round
            while (playing) {
                System.out.println(sokoban.sokobanToString());
                System.out.println("Where do you want to go? (N/E/S/W or X to exit)");
                playing = sokoban.command(in.readLine().toUpperCase());
                //ends the game if won
                if (sokoban.checkWinCondition()) {
                    System.out.println(sokoban.sokobanToString());
